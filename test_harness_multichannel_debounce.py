...
class Experiment:
	...
	def postprocess(self, array_of_samples, steady_state_DIO,sample_rate,packet_num,data_dir_name):
		...
		triggered2 = False
		toggle2_verified = False
		timesteps_triggered2 = 0

		debounce_timesteps = 8 #length of period that we gather data until the debounce
		debounce_requirement = 5  # number of timesteps required for a toggle to stay constant in order for it to be counted as a toggle
		sample_buf = []
		sample_buf = []
		trigger_sample = 0
		...
		for sample in array_of_samples:
			...
			#initialize sample buffer with starting value
			if i ==1:
				for index in range(0,debounce_timesteps):
					sample_buf.append(sample)

			#update sample buffer 
			
			for buf_i in range(0,len(sample_buf)-1):
				rev_i=len(sample_buf)-1-buf_i
				#print rev_i
				#shift buffer values back one, index 0 should be left untouched
				sample_buf[rev_i] = sample_buf[rev_i-1]
			
			sample_buf[0] = sample	#insert current sample at the head of buffer
			



			if ((sample & self.packet_received_channel_1) != (packet_received_initial_state & self.packet_received_channel_1) ) and (triggered1 ==False):
				triggered1 = True
				trigger_sample1 = sample
				trigger_i1 = i
				toggle1_verified = False
	

			if ((sample & self.packet_received_channel_2) != (packet_received_initial_state & self.packet_received_channel_2) ) and (triggered2 ==False):
				triggered2 = True
				trigger_sample2 = sample
				trigger_i2 = i
				toggle1_verified = False

			if triggered1:
				timesteps_triggered1 += 1

				if timesteps_triggered1 == debounce_timesteps:
					#reset timesteps counter
					timesteps_triggered1 = 0
					toggle1_stable = True

					for j in range(0,debounce_requirement-1):
						
					 	if (sample_buf[0] & self.packet_received_channel_1) != (sample_buf[j] & self.packet_received_channel_1):
							toggle1_stable= False
							print "toggle not stable"
					...
					if toggle1_stable:
						...
						#check toggle
						if (sample_buf[0] & self.packet_received_channel_1)== (trigger_sample1& self.packet_received_channel_1):
							toggle1_verified = True
						else:
							debounce_reject = True
							print "toggle bad"
					else:
						debounce_reject = True

			if (packet_1_received == False) and toggle1_verified:
				#block is first entered for first sample offset where pin changed state
				#latency = (i / 100.0)
				latency1 = (trigger_i1 / float(sample_rate))
			...
			if triggered2:
				timesteps_triggered2 += 1			
				if timesteps_triggered2 == debounce_timesteps:
					#reset timesteps counter
					timesteps_triggered2 = 0
					toggle2_stable = True
					...
					for j in range(0,debounce_requirement-1):
						 ...
						 if (sample_buf[0] & self.packet_received_channel_2) != (sample_buf[j] & self.packet_received_channel_2):
							toggle2_stable= False
							print "toggle not stable"
					...
					if toggle2_stable:
						...
						#check toggle
						if (sample_buf[0] & self.packet_received_channel_2) == (trigger_sample2 & self.packet_received_channel_2):
							toggle2_verified = True
						else:
							debounce_reject = True
							print "toggle bad"
					else:
			...
			if (packet_2_received == False) and toggle2_verified:
				#block is first entered for first sample offset where pin changed state
				#latency = (i / 100.0)
				latency2 = (trigger_i2 / float(sample_rate))
		...
		#if((latency1 < 1.1 and latency1>1.4) or (latency2 < 1.1 and latency2>1.4) ):
...
if __name__ == '__main__':
	...
	try:
		my_experiment.run(200000,sample_length)
